<?php
/**
 * Add login credentials to students table
 */
...
try {
    ...
    // Add username, email, password, role fields to students table
    $sql = "ALTER TABLE students 
            ADD COLUMN username VARCHAR(50) UNIQUE AFTER id,
            ADD COLUMN email VARCHAR(100) UNIQUE AFTER username,
            ADD COLUMN password VARCHAR(255) NOT NULL AFTER email,
            ADD COLUMN role ENUM('student') NOT NULL DEFAULT 'student' AFTER password,
    ...
    $pdo->exec($sql);
    echo "Successfully added login credentials to students table.\n";
