<template>
  ...
  >
    ...
    >
      ...
      <div v-if="!readMode" class="relative flex-shrink-0 bg-indigo-600 px-3 sm:px-6 py-2 sm:py-3">
        <div class="flex items-start justify-between gap-3">
          <div class="min-w-0 flex-1">
            <h2 class="text-sm sm:text-lg font-bold text-white leading-tight truncate">{{ resource.title }}</h2>
            <div class="flex flex-wrap items-center gap-2 mt-0.5 text-xs text-indigo-100">
              <span v-if="resource.subject_name" class="px-2 py-0.5 rounded-full bg-white/20 font-medium">{{ resource.subject_name }}</span>
            ...
            <!-- Reading progress - same "status readout sharing the title's own bar" pattern as
                 the eLibrary/eNotes readers, so it's visible without looking down at the footer. -->
            <div v-if="bookImages.length > 0" class="hidden sm:flex items-center gap-2 mt-1.5 max-w-xs">
              ...
              <span class="text-[11px] text-indigo-100 flex-shrink-0">{{ currentPage }}/{{ totalPages }}</span>
            </div>
            <!-- Large item banks reveal a first batch of pages immediately rather than blocking
                 on the whole document - this says the rest is still coming in the background,
                 since a reader who flips past the revealed pages would otherwise see nothing and
                 not know why. -->
            <div v-if="bookImagesLoadingMore" class="flex items-center gap-1.5 mt-1 text-[11px] text-indigo-100">
          ...
          <div class="flex items-center gap-2 flex-shrink-0">
            <!-- Read Mode - whole screen given over to the book, floating overlay controls
                 replace every other piece of chrome. Student-only, same as eLibrary/eNotes. -->
            <button
              v-if="bookImages.length > 0 && isStudentRole"
              @click="enterReadMode"
              class="flex items-center gap-1.5 px-3 py-2 bg-white text-indigo-700 font-medium text-sm rounded-lg hover:bg-indigo-50 transition-colors shadow-sm"
            >
            ...
            <!-- Reading focus overlay picker - a colored tint over the page. Shares its
                 preference key with the eLibrary/eNotes readers, so a choice made in one carries
                 over to all of them. -->
      ...
      <div class="flex-1 flex overflow-hidden relative">
        ...
        <div
          ...
          :class="[
            ...
            // available height is just a short scroll away instead of unreachable content.
        ...
        >
          ...
          <div v-else-if="preparing" class="w-full max-w-xs text-center">
            ...
            <div class="h-1.5 rounded-full bg-gray-300 dark:bg-gray-700 overflow-hidden">
              <div class="h-full bg-indigo-600 transition-all" :style="{ width: `${totalPages ? (prepared / totalPages) * 100 : 0}%` }"></div>
            </div>
          </div>
          <div
            v-else
        ...
        <!-- Right-side contents panel - the PDF's own bookmarks/outline when it has one, else a
             ...
             Hidden entirely in Read Mode, same as the eLibrary/eNotes readers' TOC sidebar. -->
        <div
...
// Shared with the eLibrary/eNotes readers too - these are preferences about the reading
// experience itself, not this one document, so they should stay in sync everywhere.
...
const pdfUrl = computed(() => resolveAssetUrl(props.resource.file_path))

const {
  loading, error, pdfDoc, currentPage, totalPages, scale,
  loadPdf, renderPage
} = usePdfRenderer(pdfUrl, {
  // No live single-page canvas in this viewer - every page is pre-rendered once into a flipbook
  // image (see prepareBook below), so the composable never needs a persistent canvas.
  getCanvasEl: () => null,
  getWrapperEl: () => null
})

const teacherName = computed(() => {
  if (!props.resource.teacher_first_name) return ''
  return `${props.resource.teacher_first_name} ${props.resource.teacher_last_name || ''}`.trim()
})

// A real drag-to-curl flipbook needs every page as a picture up front, rendered once at a fixed,
// readable resolution rather than per-zoom-level - matches the eLibrary reader's own approach.
...
// Reading focus overlay - shared preference key with the eLibrary/eNotes readers (espace:reading-
// tint), so picking a tint in one reader carries over to the others.
...
// Pages render several at a time via a small worker pool (each with its own canvas) rather than
// strictly one-at-a-time on a shared canvas - meaningfully cuts wall-clock prep time without
// touching render scale or JPEG quality. Matches the eLibrary reader's own approach.
const PREPARE_CONCURRENCY = 3

// Item bank documents can run to hundreds of pages (a compiled bank of past papers, not a single
// textbook) - blocking the whole "preparing" screen on every single page turns what should be a
// few-second wait into minutes. Instead, reveal the book to the reader as soon as a first small
// batch is ready, then keep rendering the rest in the background, periodically growing the pages
// BookFlipbook can show. bookImagesLoadingMore stays true for as long as that background work is
// still running, so the header can show a subtle "still loading" indicator.
  ...
  try {
    ...
    const renderOnePage = async (pageNum: number, canvas: HTMLCanvasElement) => {
      try {
        ...
        console.error(`Item Bank PDF: failed to render page ${pageNum}`, pageErr)
    ...
    console.error('Item Bank PDF: failed to prepare book', err)
...
// Table of contents: the PDF's own embedded bookmarks/outline when it has one, resolved down to
// a 1-indexed page number per entry; falls back to a plain page-number list for documents with
// no outline, so "jump to a topic" always has something to jump to.
interface TocEntry {
  title: string
  page: number | null
  depth: number
...
async function loadToc() {
  ...
  try {
    ...
    console.error('Item Bank PDF: failed to load table of contents', err)
    tocEntries.value = []
  }
}
...
// actually changes - covers this fullscreen transition, the plain "Fullscreen" header button, and
// Read Mode uniformly, so nothing extra is needed here beyond tracking state.
