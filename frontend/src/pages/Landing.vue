<template>
  <div class="min-h-screen bg-white font-sans text-slate-900 antialiased dark:bg-slate-950 dark:text-white">
    <!-- Nav -->
    <header class="fixed inset-x-0 top-0 z-50 border-b border-slate-200 bg-white/80 backdrop-blur-xl dark:border-white/10 dark:bg-slate-950/80">
            ...
            <div class="rounded-3xl border border-slate-200 bg-white p-5 shadow-2xl dark:border-white/10 dark:bg-slate-900">
              ...
              <div class="grid grid-cols-3 gap-3 mb-3">
                <div v-for="tile in previewTiles" :key="tile.label" class="rounded-xl bg-slate-50 p-3 ring-1 ring-slate-100 dark:bg-white/[0.06] dark:ring-white/10">
                  <div class="mb-2 h-7 w-7 rounded-lg" :class="tile.color"></div>
    ...
    <!-- About -->
    <section id="about" class="relative overflow-hidden bg-slate-100 py-20 dark:bg-slate-900 sm:py-28">
      <div class="relative mx-auto grid max-w-7xl grid-cols-1 items-center gap-16 px-4 sm:px-6 lg:grid-cols-2 lg:px-8">
        <div class="text-center lg:text-left">
          <div class="mx-auto mb-6 inline-flex h-14 w-14 items-center justify-center rounded-2xl bg-white ring-1 ring-slate-200 dark:bg-white/10 dark:ring-white/20 lg:mx-0">
            <svg class="h-7 w-7 text-slate-700 dark:text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <!-- Features -->
    <section id="features" class="relative bg-white py-20 dark:bg-slate-950 sm:py-28">
      <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div class="mt-16 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
          <div
            ...
            :key="feature.title"
            class="group rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition duration-300 hover:-translate-y-1 hover:border-slate-300 hover:shadow-lg dark:border-slate-800 dark:bg-slate-900 dark:hover:border-slate-700"
          >
            <div class="mb-4 flex h-11 w-11 items-center justify-center rounded-xl text-white shadow-sm" :class="feature.color">
              <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" :d="feature.icon"/>
              </svg>
            </div>
            <h3 class="text-sm font-semibold text-slate-900 dark:text-white">{{ feature.title }}</h3>
            <p class="mt-1.5 text-xs leading-5 text-slate-500 dark:text-slate-400">{{ feature.description }}</p>
          </div>
        </div>
      </div>
    </section>
    ...
    <!-- Roles -->
    <section id="roles" class="relative overflow-hidden bg-slate-100 py-20 dark:bg-slate-900 sm:py-28">
      <div class="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div class="mt-16 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
          <div
            v-for="role in roles"
            :key="role.title"
            class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition duration-300 hover:-translate-y-1 hover:border-slate-300 hover:shadow-lg dark:border-white/10 dark:bg-white/[0.04] dark:hover:border-white/20 dark:hover:bg-white/[0.07]"
          >
            <div class="mb-4 flex h-11 w-11 items-center justify-center rounded-xl text-white" :class="role.color">
              <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" :d="role.icon"/>
              </svg>
            </div>
            <h3 class="text-base font-semibold text-slate-900 dark:text-white">{{ role.title }}</h3>
            <p class="mt-2 text-sm leading-6 text-slate-500 dark:text-slate-400">{{ role.description }}</p>
          </div>
        </div>
      </div>
    </section>
    ...
    <!-- CTA band -->
    <section class="relative overflow-hidden bg-white py-16 dark:bg-slate-950 sm:py-20">
    ...
    <footer class="border-t border-slate-200 bg-white py-8 dark:border-white/10 dark:bg-slate-950">
      <div class="mx-auto flex max-w-7xl flex-col items-center justify-between gap-4 px-4 sm:flex-row sm:px-6 lg:px-8">
        ...
        <p class="text-xs text-slate-500 dark:text-slate-400">Empowering learners for lifelong learning.</p>
      </div>
    </footer>
  </div>
</template>
...
const previewTiles = [
  { label: 'Classes', value: '12', color: 'bg-indigo-600' },
  { label: 'Students', value: '486', color: 'bg-emerald-600' },
  { label: 'Live now', value: '3', color: 'bg-amber-600' }
]

const chartBars = [45, 70, 55, 85, 60, 95, 75]

// Each module keeps one solid, distinct color - the only place color is allowed to vary; every
// other element on the page (cards, backgrounds, text) stays neutral gray/slate.
