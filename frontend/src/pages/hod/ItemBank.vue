<template>
  <div>
    ...
    <p class="text-xs text-gray-500 dark:text-gray-400 mb-4">Moderate assessment resources uploaded by teachers in your department</p>
    ...
    <div v-if="!loading" class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
      <button
        @click="statusFilter = ''; fetchResources()"
      ...
      <button
        @click="statusFilter = 'draft'; fetchResources()"
        class="relative bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border transition-shadow hover:shadow-md text-center"
      ...
      <button
        @click="statusFilter = 'published'; fetchResources()"
        class="relative bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border transition-shadow hover:shadow-md text-center"
      ...
      <button
        @click="statusFilter = 'archived'; fetchResources()"
        class="relative bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border transition-shadow hover:shadow-md text-center"
    ...
    <!-- Empty -->
    <div v-else-if="resources.length === 0" class="text-center py-16 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700">
      ...
      <p class="text-gray-500 dark:text-gray-400 mb-3">
        {{ search || statusFilter ? 'No resources match your filters' : 'No item bank resources uploaded in your department yet' }}
      </p>
        ...
        <div class="p-5">
          <div class="flex items-start justify-between mb-3 gap-2">
            ...
            <span
              ...
              :class="resource.status === 'published' ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-300' :
                resource.status === 'draft' ? 'bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-300' :
...
const search = ref('')
const statusFilter = usePersistedRef('hod-itembank-status-filter', '')
const previewResource = ref<ItemBankResource | null>(null)

let searchTimer: number | null = null
const debouncedSearch = () => {
  if (searchTimer) window.clearTimeout(searchTimer)
  searchTimer = window.setTimeout(() => fetchResources(), 350)
}
...
const fetchResources = async () => {
  loading.value = true
  try {
    const response = await apiService.get('/hod/itembank', {
...
const changeStatus = async (resource: ItemBankResource, status: string) => {
  try {
    await apiService.put(`/hod/itembank/${resource.id}`, { status })
...
const deleteResource = async (resource: ItemBankResource) => {
  ...
  try {
    await apiService.delete(`/hod/itembank/${resource.id}`)
    await fetchResources()
...
const visibleIds = computed(() => resources.value.map(r => r.id))

const bulkSetStatus = async (status: 'draft' | 'published' | 'archived') => {
  const ids = bulk.selectedArray()
  if (ids.length === 0) return
  try {
    await apiService.post('/hod/itembank/bulk-status', { ids, status })
...
const bulkDeleteSelected = async () => {
  ...
  try {
    await apiService.post('/hod/itembank/bulk-delete', { ids })
...
const bulkExport = async () => {
  const ids = bulk.selectedArray()
  try {
    const response = await apiService.post('/hod/itembank/bulk-export', { ids }, { responseType: 'blob' })
    downloadBlob(response.data as unknown as Blob, 'item-bank.csv')
  } catch (error) {
