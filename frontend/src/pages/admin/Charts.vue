    ...
    <div class="flex flex-wrap items-center justify-between gap-3 mb-6">
      <div>
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
          Reading, watching and attendance across the whole school
        </p>
      </div>
      <select v-model="departmentId" @change="load" class="px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg dark:bg-gray-700 dark:text-white min-w-[200px]">
...
async function loadDepartments() {
  try {
    ...
    if (response.data.success) departments.value = response.data.data.departments || response.data.data || []
  } catch (err) {
...
async function load() {
  ...
  error.value = null
  try {
    const response = await apiService.get('/admin/analytics/engagement', {
      params: departmentId.value ? { department_id: departmentId.value } : {}
    })
