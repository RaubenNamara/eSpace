<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      <div>
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
          Reading, watching and attendance for your own class(es)
        </p>
      </div>
    </div>
    ...
    <div v-else-if="students.length === 0" class="card text-center text-gray-500 dark:text-gray-400 py-12">
      No class is attached to you yet in this department - once you're set as a class teacher, or
      you set an assignment for a class, its students' engagement will show up here.
    </div>
...
async function load() {
  ...
  error.value = null
  try {
    const response = await apiService.get('/teacher/analytics/engagement')
