<template>
  <div class="p-4 sm:p-6">
    <div class="mb-6 flex items-center gap-3">
      ...
      <div>
        ...
        <p class="text-sm text-gray-600 dark:text-gray-400 mt-0.5">
          EOC building blocks defined by your school - browse and view what's been set for your subjects.
        </p>
      </div>
    </div>
    ...
    <div v-else-if="constructs.length === 0" class="text-center py-16 bg-white dark:bg-gray-800 rounded-xl border border-dashed border-gray-300 dark:border-gray-700 text-gray-400">
      No constructs found for your enrolled department(s) yet.
    </div>
...
// Subject filter options are derived from every construct visible to this student (fetched once,
...
async function loadAllConstructsForFilterOptions() {
  try {
    const response = await apiService.get('/student/constructs')
...
async function openDetail(id: number) {
  ...
  try {
    const response = await apiService.get(`/student/constructs/${id}`)
