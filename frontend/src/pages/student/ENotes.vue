    ...
    <template v-if="!activeSubjectId">
      ...
      <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-indigo-600 via-purple-600 to-fuchsia-600 shadow-lg shadow-indigo-500/20 p-4 sm:p-5 mb-6">
        ...
        <div class="relative flex items-center gap-3 mb-3">
          ...
          <div class="min-w-0">
            <h1 class="text-lg sm:text-2xl font-bold text-white leading-tight">eNotes</h1>
            <p class="text-xs sm:text-sm text-indigo-100">Study notes from your teachers, with AI voice narration</p>
          </div>
        </div>
      ...
      <div v-if="loading" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
        <div v-for="i in 8" :key="i" class="animate-pulse">
          <div class="h-40 rounded-2xl bg-gray-200 dark:bg-gray-700"></div>
        </div>
      </div>
      ...
      <template v-else-if="filteredSubjectGroups.length > 0">
        <h2 class="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-4">Browse by Subject</h2>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
          ...
          >
            ...
            <div class="relative flex items-start justify-between mb-4">
              ...
              <svg class="w-5 h-5 text-gray-300 dark:text-gray-600 group-hover:text-amber-500 group-hover:translate-x-0.5 transition-all flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
              </svg>
            </div>
            <h3 class="relative text-lg font-semibold text-gray-900 dark:text-white mb-2 group-hover:text-amber-600 dark:group-hover:text-amber-400 transition-colors line-clamp-1">
              {{ group.name }}
            </h3>
            <span
      ...
      <div v-else class="text-center py-16 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700">
        ...
        <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-2">
          {{ subjectGroups.length === 0 ? 'No eNotes yet' : 'No subjects match your search' }}
        </h3>
    ...
    <template v-else>
      <div class="flex items-center gap-2 mb-2">
        <button
          ...
          class="inline-flex items-center gap-1.5 text-sm font-medium text-gray-600 dark:text-gray-300 hover:text-amber-600 dark:hover:text-amber-400 transition-colors"
        >
      ...
      <div class="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p class="text-xs text-gray-500 dark:text-gray-400">{{ activeSubjectTopics.length }} {{ activeSubjectTopics.length === 1 ? 'topic' : 'topics' }}</p>
        </div>
      </div>

      <div v-if="activeSubjectTopics.length > 0" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <RouterLink
        ...
        >
          ...
          <div class="p-5">
            <div class="flex items-start justify-between gap-2 mb-3">
              ...
              <span
                v-if="topic.narration_voice"
                class="flex-shrink-0 inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold bg-purple-50 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300"
                title="Voice narration available"
              >
                🔊 Audio
              </span>
            </div>
            <h3 class="text-sm font-semibold text-gray-900 dark:text-white line-clamp-2 mb-1 group-hover:text-amber-600 dark:group-hover:text-amber-400 transition-colors">{{ topic.title }}</h3>
            <p v-if="topic.description" class="text-xs text-gray-500 dark:text-gray-400 line-clamp-2 mb-3">{{ topic.description }}</p>
            <div class="flex items-center justify-between text-xs text-gray-400 dark:text-gray-500">
              <span class="inline-flex items-center gap-1">
                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                {{ topic.total_pages }} {{ topic.total_pages === 1 ? 'page' : 'pages' }}
              </span>
              <span v-if="topic.teacher_first_name" class="truncate">{{ topic.teacher_first_name }} {{ topic.teacher_last_name }}</span>
            </div>
          </div>
        </RouterLink>
      </div>
      ...
      <div v-else class="text-center py-16 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700">
        <p class="text-gray-500 dark:text-gray-400">No eNotes topics in this subject yet.</p>
      </div>
    </template>
  </div>
</template>
...
const filteredSubjectGroups = computed(() => {
  ...
  return subjectGroups.value.filter(group => {
    ...
    return group.topics.some(topic => topic.title.toLowerCase().includes(q) || (topic.description || '').toLowerCase().includes(q))
  })
})

const activeSubject = computed(() => subjectGroups.value.find(g => g.id === activeSubjectId.value) || null)
const activeSubjectTopics = computed(() => activeSubject.value?.topics || [])
